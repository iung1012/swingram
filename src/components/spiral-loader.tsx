...
export function SpiralLoader({ size = 16, className }: SpiralLoaderProps) {
  ...
  const handleFastComplete = React.useCallback(() => {
    ...
    if (repeatCountRef.current < FAST_REPEATS) fastRef.current?.goToAndPlay(0, true);
    else startSlowPhase();
  }, [startSlowPhase]);

  const handleSlowComplete = React.useCallback(() => {
    repeatCountRef.current += 1;
    if (repeatCountRef.current < SLOW_REPEATS) slowRef.current?.goToAndPlay(0, true);
    else startFastPhase();
  }, [startFastPhase]);

  if (!isMounted) {
    return <div className={cn("inline-block", className)} style={{ width: size, height: size }} aria-label="Carregando" />;
  }
  const needsInvert = !isDark;
  const style: React.CSSProperties = { width: size, height: size, filter: needsInvert ? "invert(1)" : undefined };

  return (
    <span role="status" aria-label="Carregando" className={cn("relative inline-block", className)} style={{ width: size, height: size }}>
      <span style={{ position: "absolute", inset: 0, opacity: phase === "fast" ? 1 : 0 }}>
        <Lottie lottieRef={fastRef} animationData={spiralFastData} loop={false} autoplay onComplete={handleFastComplete} style={style} />
      </span>
      <span style={{ position: "absolute", inset: 0, opacity: phase === "slow" ? 1 : 0 }}>
        <Lottie lottieRef={slowRef} animationData={spiralSlowData} loop={false} autoplay={false} onComplete={handleSlowComplete} style={style} />
      </span>
    </span>
  );
}

export function SpiralLoaderBlock({ label = "Carregando", size = 28, className }: { label?: string; size?: number; className?: string }) {
  return (
    <div className={cn("flex flex-col items-center justify-center gap-3 py-10 text-xs text-muted-foreground", className)}>
